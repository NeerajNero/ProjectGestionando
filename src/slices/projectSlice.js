...
export const getProjects = createAsyncThunk('getProjects', async() => {
    ...
    const response = await axios.get('http://localhost:5000/api/projects/project',{
        headers: {
...
export const addProject = createAsyncThunk('addProject', async({name,description}) => {
    const token = localStorage.getItem('authToken')
    const response = await axios.post('http://localhost:5000/api/projects/addProject', {name,description}, {
        headers: {
            Authorization: `Bearer ${token}`
        }
    })

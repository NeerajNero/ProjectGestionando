...
export const addTask = createAsyncThunk('addTask', async(taskData) => {
    const token = localStorage.getItem('authToken')
    const response = await axios.post('http://localhost:5000/api/tasks/addTask', {taskData}, {
        headers: {
            Authorization: `Bearer ${token}`
        }
    })
...
export const updateTask = createAsyncThunk('updateTask', async({taskId}) => {
    const token = localStorage.getItem('authToken')
    const response = await axios.put('http://localhost:5000/api/tasks/updateTask', {taskId}, {
        headers: {
            Authorization: `Bearer ${token}`
        }
    })
